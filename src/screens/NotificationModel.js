...
function NotificationModel() {
  ...
  const [deletedPostIds, setDeletedPostIds] = useState([]); // Store deleted post IDs
  ...
  const fetchNotification = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Filter out notifications related to deleted posts
        const filteredData = response.data.filter(notification => !deletedPostIds.includes(notification.postId));
